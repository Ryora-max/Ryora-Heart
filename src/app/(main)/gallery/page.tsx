...
export default function GalleryPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-100 via-pink-100 to-rose-100 p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8 text-center">
          <h1 className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-purple-500 to-pink-600 bg-clip-text text-transparent mb-2">
            📸 Gallery
          </h1>
        ...
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 bg-white/80 border-2 border-purple-200 rounded-xl text-sm focus:border-purple-400 focus:outline-none w-48"
            />
          </div>
          <div className="flex gap-2">
            <button onClick={() => setViewMode("masonry")} className={`px-3 py-2 rounded-lg text-sm transition-all ${viewMode === "masonry" ? "bg-purple-500 text-white" : "bg-white/80 text-purple-600 hover:bg-white"}`}>Masonry</button>
            <button onClick={() => setViewMode("grid")} className={`px-3 py-2 rounded-lg text-sm transition-all ${viewMode === "grid" ? "bg-purple-500 text-white" : "bg-white/80 text-purple-600 hover:bg-white"}`}>Grid</button>
        ...
        {loading ? (
          ...
          <div className={`gallery-grid ${viewMode === "masonry" ? "columns-1 md:columns-2 lg:columns-3 gap-4 space-y-4" : "grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4"}`}>
        ...
        {selectedPhoto && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-xl p-4" onClick={() => setSelectedPhoto(null)}>
              ...
              <button onClick={() => setSelectedPhoto(null)} className="absolute -top-12 right-0 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-colors">
